    ...
    const TimeTicker  = ({endAt, now, progress}:{endAt: string, now: number, progress:number}) =>
    {
      ...
      if(remaining <=  0)
      {
        return (
            ...
            <span className="text-[10px] font-bold text-gray-500">Time Left</span>
      ...
      const colorClass = (progress >= 75) ? "bg-red-100 text-red-600" : progress >= 50 ? "bg-yellow-100 text-yellow-600" : "bg-green-100 text-green-600";
      return(
        <div className="flex items-center justify-between mt-1">
          <span className="text-[8px] font-semibold text-gray-500">Time Left</span>
            <span className={cn("text-[9px] font-bold px-2 py-0.5 rounded-full", colorClass)}>
            {timeString}
          </span> 
        </div>
      )
        ...
        <Draggable draggableId={`todo-${todo.id}`} index={index}>
          {(p) => (
            <div ref={p.innerRef} {...p.draggableProps} {...p.dragHandleProps}>
             <Card
  className={cn(
    "flex flex-col justify-between h-[280px] rounded-xl shadow-md border border-gray-100", // ← h-260 to h-280
    getCardBg(todo, now),
  )}
>
  <CardContent className="p-6 flex flex-col h-full justify-between gap-4"> {/* ← p-5 to p-6, gap-3 to gap-4 */}

    {/* Title + Badge + Delete */}
    <div className="flex flex-col gap-2.5"> {/* ← gap-2 to gap-2.5 */}
      <div className="flex justify-between items-start">
        <h3
          className="font-bold text-[13px] leading-snug text-gray-800"
          title={todo.title}
        >
        ...
        <div className="flex items-center gap-1.5">
          ...
          <Button
            ...
            className="rounded-full h-5 w-5 text-[8px] text-white bg-red-500 hover:bg-red-600 p-0"
          >
    ...
    <div className="flex justify-between items-center">
      <span className="flex flex-col gap-1"> {/* ← gap-0.5 to gap-1 */}
      ...
      {todo.endAt && (
        <span className="flex flex-col gap-1 items-end"> {/* ← gap-0.5 to gap-1 */}
          <span className="text-[9px] font-semibold text-gray-400 uppercase tracking-wide">
            End
          </span>
          <span className="text-[12px] font-semibold text-gray-700">
    ...
    {todo.status === "completed" && todo.completedAt && (
      <div className="flex justify-between items-center bg-green-50 rounded-lg px-3 py-1.5"> {/* ← px-2 to px-3, py-1 to py-1.5 */}
    ...
    {progress && todo.endAt && (
      <div className="flex flex-col gap-2"> {/* ← gap-1.5 to gap-2 */}
        <TimeTicker
          endAt={todo.endAt}
          now={now}
