import type { Todo, TodoStatusEnum } from "@repo/shared";
import { format } from "date-fns";
import { Text, TouchableOpacity, View } from "react-native";
import type z from "zod";
import { useDeleteTodo, useUpdateTodoStatus } from "@/hooks/api-hooks";
...
function TimeTicker({ endAt, now }: { endAt: string; now: number }) {
  ...
  if (remaining <= 0) {
    return (
      <View className="mt-3 flex-row items-center justify-between">
        <Text className="text-[#9CA3AF] text-xs">Time Left</Text>
        <View className="rounded-full bg-red-900 px-3 py-1">
          <Text className="font-bold text-[#F87171] text-xs">Expired!</Text>
        </View>
      </View>
    );
  }
  ...
  const colorBg =
    progress >= 75 ? "#450a0a" : progress >= 50 ? "#422006" : "#052e16";
  const colorText =
    progress >= 75 ? "#F87171" : progress >= 50 ? "#FBBF24" : "#34D399";
  ...
  return (
    <View className="mt-3 flex-row items-center justify-between">
      ...
      >
        <Text className="font-bold text-xs" style={{ color: colorText }}>
          {timeStr}
        </Text>
      </View>
    </View>
...
export default function TodoCard({ todo, now }: { todo: Todo; now: number }) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <View style={{ flexDirection: "row", alignItems: "center", gap: 8 }}>
          ...
          <TouchableOpacity
            ...
            onPress={() =>
              deleteTodo.mutate({ params: { path: { id: todo.id } } })
            }
            style={{
              width: 28,
              height: 28,
      ...
      <View
        style={{ height: 1, backgroundColor: "#2A2A2A", marginBottom: 16 }}
      />
